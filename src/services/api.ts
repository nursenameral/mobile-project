...
// Yeni Fiş Ekleme (Create Receipt)
export const createReceipt = async (token: string, data: any): Promise<ApiResponse<Receipt>> => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(resData.message || 'Fiş oluşturulamadı');
    }
...
// Yeni Fatura Ekleme (Create Invoice)
export const createInvoice = async (
  token: string, 
  data: { baslik: string; tutar: number; tur: string; son_odeme_tarihi: string }
